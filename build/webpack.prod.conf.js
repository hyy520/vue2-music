...
var path = require('path') // 引入nodejs的路径模块
var utils = require('./utils') // utils工具配置文件，主要用来处理css类文件的loader
var webpack = require('webpack') // 引入webpack，来使用webpack内置插件
var config = require('../config') // config目录下的index.js配置文件，主要用来定义了生产和开发环境的相关基础配置
var merge = require('webpack-merge') // webpack的merger插件，主要用来处理配置对象合并的，可以将一个大的配置对象拆分成几个小的，合并，相同的项将覆盖
var baseWebpackConfig = require('./webpack.base.conf') // webpack.base.conf.js配置文件，我其他博客文章已经解释过了，用来处理不同类型文件的loader
var CopyWebpackPlugin = require('copy-webpack-plugin') // copy-webpack-plugin使用来复制文件或者文件夹到指定的目录的
var HtmlWebpackPlugin = require('html-webpack-plugin') // html-webpack-plugin是生成html文件，可以设置模板，之前的文章将过了
var ExtractTextPlugin = require('extract-text-webpack-plugin') // extract-text-webpack-plugin这个插件是用来将bundle中的css等文件产出单独的bundle文件的，之前也详细讲过
var OptimizeCSSPlugin = require('optimize-css-assets-webpack-plugin') // optimize-css-assets-webpack-plugin插件的作用是压缩css代码的，还能去掉extract-text-webpack-plugin插件抽离文件产生的重复代码，因为同一个css可能在多个模块中出现所以会导致重复代码，换句话说这两个插件是两兄弟
